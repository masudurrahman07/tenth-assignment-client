      ...
      <div className="p-5 flex flex-col h-full">
        ...
        <div className="mt-auto">
          ...
          <div className="flex gap-2">
            <Link
              to={`/allJobs/${job._id}`}
              className="flex-1 text-center bg-linear-to-r from-blue-700 via-sky-600 to-blue-500 text-white text-sm font-medium py-2 rounded-lg transition-colors duration-200">View Details</Link>

            {!isOwner && currentUserEmail && (
              <Link
                to={`/allJobs/${job._id}`}
                className="flex-1 text-center bg-linear-to-r from-green-400 via-green-500 to-green-600  text-white text-sm font-medium py-2 rounded-lg transition-colors duration-200">Accept </Link> )}
          </div>
        </div>
      </div>
    </div>
